 ...
 *
 * Arduino Sketch to show data being sent between a Raspberry Pi and Arduino using SMBus and Wire.h
 * Raspberry Pi is the Master and Arduino is a Slave
 * We have chosen the slave address for the Arduino to be the first 'safe' unreserved address (i.e. 4)
 *
 * Arduino reads the voltage sensors and allows the Raspberry Pi to interrogate the Arduino
 * Arduino can also be interrogated if it is safe for the Raspberry Pi to send it another chunk of image data
 *
...
void loop(void) {
  ...
  /*if (dataReady == READY) {
    ...
    if (abs(millis() - broadcastTime) >= BROADCASTDELAY) {
      ...
      dataReady = PENDING;
      //Serial.print(F("type = 0x")); Serial.println(sensorMsg.type, HEX);
      //Serial.print(F("bat_rpi = ")); Serial.println(sensorMsg.bat_rpi);
